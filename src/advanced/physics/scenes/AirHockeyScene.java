...
//		ball.setFillColor(new MTColor(160,160,160,255));
        ...
        MTRectangle leftSide = new MTRectangle(
                ...
                PhysicsHelper.scaleDown(0, scale), PhysicsHelper.scaleDown(app.width / 2f, scale), PhysicsHelper.scaleDown(app.height, scale));
        ...
        MTRectangle rightSide = new MTRectangle(
                ...
                PhysicsHelper.scaleDown(0, scale), PhysicsHelper.scaleDown(app.width, scale), PhysicsHelper.scaleDown(app.height, scale));
...
//				/*
                ...
                if (userData1 instanceof IPhysicsComponent && userData2 instanceof IPhysicsComponent)
                { //Check for ball/star collision
...
//								triggerSound(goalHit);
                                ...
                                } else
                                {
                                    ...
                                    //Reset ball
                                    if (theBall.getUserData("resetted") == null)
                                    { //To make sure that we call destroy only once
...
//											a.getBody().setLinearVelocity(new Vec2(0,0));
                ...
                { //if at lest one if the colliding bodies' userdata is not a physics shape
...
//				*/
    ...
    private void reset()
    {
        if (ball.getUserData("resetted") == null)
        { //To make sure that we call destroy only once
